...
showGameState g =
  "\n" ++
  headSpace (board g) ++
  getHeader (size (board g)) ++ stringScore (checkScore (board g)) ++ "\n" ++ getTable (board g) 0
...
getTable board row =
  if row < size board
    then getTableRow board (range ((0, row), (size board - 1, row))) ++ "\n" ++ getTable board (row + 1)
    else ""
...
getTableRow board ((xPos, yPos):xs)
  | xPos == 0 =
    if size board > 9 && (yPos + 1) < 10
      then show (yPos + 1) ++ "  " ++ getCell (xPos, yPos) (pieces board) : ' ' : getTableRow board xs
      else show (yPos + 1) ++ ' ' : getCell (xPos, yPos) (pieces board) : ' ' : getTableRow board xs
